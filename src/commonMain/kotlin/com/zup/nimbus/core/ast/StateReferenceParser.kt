...
class StateReferenceParser(private val logger: Logger) {
  private fun pathError(path: String): Literal {
    logger.error("invalid path \"$path\". Please, make sure your variable names contain only letters, " +
  ...
  private fun stateIdError(id: String): Literal {
    logger.error("The referred state is invalid because it uses a key word as its id: $id. Using null in its place.")
    return Literal(null)
  }

  private fun stateNotFoundError(id: String): Literal {
    logger.error("Couldn't find state with id \"$id\". Using null in its place.")
    return Literal(null)
  }
